...
for i in range(len(traitgroups[traitgroup_num - 1])):
    ...
    SSE = pd.read_csv('/gpfs/gibbs/pi/zhao/yj348/sse/Primary_Summary_Statistics/{0}/{0}.clumped_SSE.tsv'.format(trait), sep = '\t')
    SSE = SSE[SSE['p_diff_adj'] < 0.05]
    non_SSE = pd.read_csv('/gpfs/gibbs/pi/zhao/yj348/sse/Primary_Summary_Statistics/{0}/{0}.clumped_all.tsv'.format(trait), sep = '\t')

    SSE_representative = SSE.loc[SSE.groupby('locus')['p_min'].idxmin()]
    SSE_representative.to_csv('/gpfs/gibbs/pi/zhao/yj348/sse/Primary_Summary_Statistics/{0}/{0}.clumped_SSE_representative.tsv'.format(trait), sep = '\t', index = False)
